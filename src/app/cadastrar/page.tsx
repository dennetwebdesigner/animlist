  ...
  function handleSubmit(e: any) {
    ...
    registerWithEmailPassword(email.value, password.value).then((result) => {
      if (result) router.back();
        ...
        alert(
          "Ocorreu um erro, verifique se esta conta já esta cadastrado e tente novamente, ou fale com o suporte!"
        );
    });
  }
  return (
    <div className="w-full h-screen">
      ...
      <main className="block sm:w-full lg:w-2/6 m-auto lg:my-[15vh]">
        <form className="w-full mb-4" onSubmit={handleSubmit}>
          <h3 className="sm:mx-auto  font-sans text-2xl font-semibold text-center md:text-left">
            Cadastrar conta
          </h3>
          <Fieldset
            data={{
          ...
          <fieldset className="flex w-full justify-end p-2 ">
            <button
              type="submit"
              className="border border-orange-400 py-2 px-5 hover:bg-orange-400 transition-[2s] hover:w-full hover:text-right hover:font-extrabold hover:text-lg"
            >
              Cadastrar
            </button>
          </fieldset>
        ...
        <div className="w-full flex justify-center border rounded-md p-2 my-4">
          <Link href="/entrar" className="text-xl ">
            Já tem uma conta? Vamos fazer o login!
          </Link>
        </div>
        ...
        <button
          ...
          onClick={() => authWithGoogle(router.back)}
        >
          Google
        </button>
      </main>
